...
BuyerGains<-function(Symbol, Expiration){
  #OptionChain<-getOptionChain(Symbol)
	#try(OptionChain<-getOptionChain(Symbol, Exp=Expiration),silent=TRUE)
...
SelectedDate<-function(buyerGains,expirationDate){
  expDate<-as.Date(buyerGains$expiration[1]);
  ifelse(expDate!=expirationDate,
         outDate<-"Invalid date specified.",
         outDate<-format(expDate,"%dd %M %yy"))
  outDate
}
...
shinyServer(
    ...
    #selectedDate<-reactive({SelectedDate(buyerGains(),input$expirationDate)})
	  output$expirationDate <- renderPrint({input$expirationDate})
	  #output$ExpirationDate <- renderPrint({selectedDate()})
    output$symbol <- renderPrint({input$symbol})
		output$mpval<-renderPrint({buyerGains()[,1][which.min(buyerGains()[,2]+buyerGains()[,3])]})
		output$plot<-renderPlot({try(barplot(t(data.matrix(buyerGains()[2:3])),
                                         col=c("red","green"),
                                         main="Buyer Gains",
